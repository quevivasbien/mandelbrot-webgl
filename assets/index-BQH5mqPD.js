...
`;function qe(e,t=["uX0","uY0","uX1","uY1"]){const i=oi.getContext("webgl",{preserveDrawingBuffer:!0});if(!i){alert("Unable to initialize WebGL. Your browser or machine may not support it.");return}const n=ci(i,fi,e),o={};for(const s of t)o[s]=i.getUniformLocation(n,s);const r={gl:i,program:n,attribLocations:{vertexPosition:i.getAttribLocation(n,"aVertexPosition")},uniformLocations:o,buffers:si(i)};return C>1&&(r.uniformLocations.uPixWidth=i.getUniformLocation(n,"uPixWidth")),r}const ai=document.getElementById("glcanvas");function li(){return C===1?`
        precision highp float;
...
`:""}
                ...
                for (int yi = 0; yi < ${C}; yi++) {
                    vec2 c_re = add_fp64(
                        ...
                        mul_fp64(
                            ...
                            div_fp64(uPixWidth, vec2(${C}.0, 0.0))
                        )
                    );
                    vec2 c_im = add_fp64(
                        cBase_im,
    ...
    `}function _i(){let e=$;const t=[e.re.toNumber()],i=[e.im.toNumber()];for(let n=0;n<D-1;n++){const o={re:e.re.times(e.re).minus(e.im.times(e.im)).plus($.re),im:e.re.times(e.im).times(2).plus($.im)};e=o,t.push(o.re.toNumber()),i.push(o.im.toNumber())}return{zReHistory:t,zImHistory:i}}function Ei(e){const{gl:t,buffers:i,program:n,attribLocations:o,uniformLocations:r}=e,s=2,u=t.FLOAT,f=!1,c=0,a=0;t.bindBuffer(t.ARRAY_BUFFER,i.position),t.vertexAttribPointer(o.vertexPosition,s,u,f,c,a),t.enableVertexAttribArray(o.vertexPosition),t.useProgram(n);const{zReHistory:l,zImHistory:d}=_i();if(t.uniform1fv(r.uZReHistory,l),t.uniform1fv(r.uZImHistory,d),t.uniform2fv(r.uDeltaOrigin,[_.x0.minus($.re),_.y0.minus($.im)]),t.uniform2fv(r.uViewSize,[_.x1.minus(_.x0),_.y1.minus(_.y0)]),e.uniformLocations.uPixWidth){const h=_.x1.minus(_.x0).div(vi.width).toNumber();t.uniform2fv(r.uPixWidth,split_float(h))}t.drawArrays(t.TRIANGLE_STRIP,0,4)}const bi=["uZReHistory","uZImHistory","uDeltaOrigin","uViewSize"];let Le;function ot(e=!1){(!Le||e)&&(Le=qe(Ni(),bi)),Ei(Le)}const H=document.getElementById("2dcanvas");let ge;function yi(e=!1){(!ge||e)&&(ge=H.getContext("2d",{willReadFrequently:!0}));const t=new Uint8ClampedArray(H.width*H.height*4),i=_.x0.toNumber(),n=_.x1.toNumber(),o=_.y0.toNumber(),r=_.y1.toNumber();for(let u=0;u<H.height;u++)for(let f=0;f<H.width;f++){const c=u*H.width+f,a=i+f*(n-i)/(H.width-1),l=r-u*(r-o)/(H.height-1),d={re:a,im:l},h={...d};for(let N=0;N<D;N++){if(h.re*h.re+h.im*h.im>4){const P=N+1-Math.log(Math.log(h.re*h.re+h.im*h.im))/Math.log(2);t[c*4+0]=255*(1-Math.cos(.02*P))/2,t[c*4+1]=255*(1-Math.cos(.03*P))/2,t[c*4+2]=255*(1-Math.cos(.05*P))/2;break}const w=h.re*h.re-h.im*h.im+d.re;h.im=2*h.re*h.im+d.im,h.re=w}t[c*4+3]=255}const s=ge.getImageData(0,0,H.width,H.height);s.data.set(t),ge.putImageData(s,0,0)}const V=document.getElementById("2dcanvas");let ae;function st(e=!1){(!ae||e)&&(ae=V.getContext("2d",{willReadFrequently:!0}));const t=new Uint8ClampedArray(V.width*V.height*4),{x0:i,y0:n,x1:o,y1:r}=_;let s=$;const u=[{re:s.re.toNumber(),im:s.im.toNumber()}];for(let d=0;d<D-1;d++){const h={re:s.re.times(s.re).minus(s.im.times(s.im)).plus($.re),im:s.re.times(s.im).times(2).plus($.im)};s=h,u.push({re:h.re.toNumber(),im:h.im.toNumber()})}const f={re:i.minus($.re).toNumber(),im:n.minus($.im).toNumber()},c=o.minus(i).toNumber(),a=r.minus(n).toNumber();for(let d=0;d<V.height;d++)for(let h=0;h<V.width;h++){const N=d*V.width+h,w=h/(V.width-1),P=1-d/(V.height-1),z=f.re+w*c,q=f.im+P*a,E={re:z,im:q},b={...E};for(let K=0;K<D;K++){const A=u[K],U={re:A.re+b.re,im:A.im+b.im};if(U.re*U.re+U.im*U.im>4){const re=K+1-Math.log(Math.log(U.re*U.re+U.im*U.im))/Math.log(2);t[N*4+0]=255*(1-Math.cos(.02*re))/2,t[N*4+1]=255*(1-Math.cos(.03*re))/2,t[N*4+2]=255*(1-Math.cos(.05*re))/2;break}const oe=2*A.re*b.re-2*A.im*b.im+b.re*b.re-b.im*b.im+E.re,ne=2*A.re*b.im+2*A.im*b.re+2*b.re*b.im+E.im;b.re=oe,b.im=ne}t[N*4+3]=255}const l=ae.getImageData(0,0,V.width,V.height);l.data.set(t),ae.reset(),ae.putImageData(l,0,0)}const le=document.getElementById("glcanvas"),Ne=document.getElementById("2dcanvas");let Z=le,Q=rt;const I={x:new B(-.5),y:new B(0),halfWidth:new B(1.5)},_={x0:new B(-2),y0:new B(-1.5),x1:new B(1),y1:new B(1.5)};function xi(){const e=1.5/I.halfWidth.toNumber(),t=3+Math.floor(Math.log(e)/Math.log(10));document.getElementById("view-loc").innerHTML=`re: ${I.x.toPrecision(t)}, im: ${I.y.toPrecision(t)}, zoom: ${e.toPrecision(3)}`}function zi(){const e=4+Math.ceil(-Math.log(I.halfWidth)/Math.log(10));for(;B.precision<e;)B.precision+=4;for(;B.precision>e+8;)B.precision-=4}function $e(){const e=I.halfWidth.times(Z.height/Z.width);_.x0=I.x.minus(I.halfWidth),_.x1=I.x.plus(I.halfWidth),_.y0=I.y.minus(e),_.y1=I.y.plus(e),xi(),zi()}let D=document.getElementById("max-iters").value,Te=document.getElementById("auto-max-iters").checked,C=parseInt(document.getElementById("anti-aliasing").value);function ut(e){e&&(D=e,document.getElementById("max-iters").value=D)}function ct(){let e=-Math.log(I.halfWidth),t=100*(e>0?Math.ceil(e/2):1);return D!==t?(ut(t),!0):!1}function R(e=!1){Te&&(e|=ct()),Q(e)}document.getElementById("auto-max-iters").addEventListener("click",()=>{Te=document.getElementById("auto-max-iters").checked,document.getElementById("max-iters").disabled=Te,ct()&&R(!0)});document.getElementById("max-iters").addEventListener("input",()=>{ut(parseInt(document.getElementById("max-iters").value)),R(!0)});document.getElementById("anti-aliasing").addEventListener("change",e=>{C=parseInt(e.target.value),R(!0)});function Fe(){Z.width=.8*window.innerWidth,Z.height=.8*window.innerHeight,console.log(`Canvas size: ${Z.width}x${Z.height}`),$e()}window.addEventListener("resize",()=>{Fe(),R(!0)});let $={re:new B(0),im:new B(0)};function Pi(e,t){const i=_.x1.minus(_.x0),n=_.y1.minus(_.y0);return{re:_.x0.plus(i.times(e)),im:_.y0.plus(n.times(t))}}function ft(e){if(Q!==st&&Q!==ot)return;const t=e.offsetX/Z.width,i=1-e.offsetY/Z.height;$=Pi(t,i),console.log(`Perturbation point: ${$.re}, ${$.im}`),R()}Ne.addEventListener("click",ft);le.addEventListener("click",ft);function se(e){e?(Z=le,Fe(),Ne.style.display="none",le.style.display="block"):(Z=Ne,Fe(),Ne.style.display="block",le.style.display="none"),document.getElementById("anti-aliasing").disabled=!e}se(!0);document.getElementById("render-type").addEventListener("change",e=>{e.target.value==="WebGL"?(Q=rt,se(!0)):e.target.value==="WebGL-64"?(Q=wi,se(!0)):e.target.value==="WebGL-Perturb"?(Q=ot,se(!0)):e.target.value==="JS"?(Q=yi,se(!1)):e.target.value==="JS-Perturb"&&(Q=st,se(!1)),R(!0)});document.getElementById("render-type").value="WebGL";document.getElementById("download-link").addEventListener("click",()=>{const e=document.getElementById("download-link");e.href=Z.toDataURL()});function me(e,t){I.x=I.x.plus(I.halfWidth.times(e)),I.y=I.y.plus(I.halfWidth.times(t)),$e()}function De(e){I.halfWidth=I.halfWidth.div(e),$e()}const S={left:!1,right:!1,up:!1,down:!1,zoomOut:!1,zoomIn:!1},L={left:"a",right:"d",up:"w",down:"s",zoomOut:"q",zoomIn:"e"};window.addEventListener("keydown",e=>{switch(e.key){case L.left:S.left=!0;break;case L.right:S.right=!0;break;case L.up:S.up=!0;break;case L.down:S.down=!0;break;case L.zoomOut:S.zoomOut=!0;break;case L.zoomIn:S.zoomIn=!0;break}});window.addEventListener("keyup",e=>{switch(e.key){case L.left:S.left=!1;break;case L.right:S.right=!1;break;case L.up:S.up=!1;break;case L.down:S.down=!1;break;case L.zoomOut:S.zoomOut=!1;break;case L.zoomIn:S.zoomIn=!1;break}});document.getElementById("up-key").innerText=L.up.toUpperCase();document.getElementById("left-key").innerText=L.left.toUpperCase();document.getElementById("down-key").innerText=L.down.toUpperCase();document.getElementById("right-key").innerText=L.right.toUpperCase();document.getElementById("zoom-out-key").innerText=L.zoomIn.toUpperCase();document.getElementById("zoom-in-key").innerText=L.zoomOut.toUpperCase();const Pe=.25,at=2;document.getElementById("zoom-out").addEventListener("click",()=>{De(1/at),R()});document.getElementById("zoom-in").addEventListener("click",()=>{De(at),R()});document.getElementById("move-up").addEventListener("click",()=>{me(0,Pe),R()});document.getElementById("move-down").addEventListener("click",()=>{me(0,-Pe),R()});document.getElementById("move-left").addEventListener("click",()=>{me(-Pe,0),R()});document.getElementById("move-right").addEventListener("click",()=>{me(Pe,0),R()});const we=.04,Xe=1.25;function Ii(){let e=0,t=0,i=1;S.left&&(e-=we),S.right&&(e+=we),S.up&&(t+=we),S.down&&(t-=we),S.zoomOut&&(i/=Xe),S.zoomIn&&(i*=Xe),(e!==0||t!==0)&&me(e,t),i!==1&&De(i),(e!==0||t!==0||i!==1)&&R()}function lt(){Ii(),requestAnimationFrame(lt)}R();lt();
